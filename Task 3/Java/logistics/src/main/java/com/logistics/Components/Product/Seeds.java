...
import com.logistics.Serialization.PBLogistics.PBSeeds;
...
public class Seeds implements Flora {
    ...
    @Override public void copyToProto(PBStockItem.Builder message){
        PBSeeds.Builder pbSeeds = PBSeeds.newBuilder();
        pbSeeds.setExpirationDays(expirationDays);

        PBFlora.Builder pbFlora = pbSeeds.getFloraBuilder();
        ...
        message.setSeeds(pbSeeds);

    };
    @Override public void copyFromProto(PBStockItem message){
        PBSeeds pbSeeds = message.getSeeds();
        name = pbSeeds.getFlora().getProduct().getName();
        cost = pbSeeds.getFlora().getProduct().getCost();
        supplier = pbSeeds.getFlora().getProduct().getSupplier();
        packaging.setUnits(pbSeeds.getFlora().getProduct().getPackaging().getUnits());
        packaging.setCostPerUnit(pbSeeds.getFlora().getProduct().getPackaging().getCostPerUnit());
        isCultivated = pbSeeds.getFlora().getIscultivated();
        expirationDays = pbSeeds.getExpirationDays();
    };
}
