...
import com.logistics.Serialization.PBLogistics.PBFlora;
import com.logistics.Serialization.PBLogistics.PBPackage;
import com.logistics.Serialization.PBLogistics.PBPlants;
...
public class Plants implements Flora {
    ...
    @Override public void copyToProto(PBStockItem.Builder message){
        PBPlants.Builder pbPlants = PBPlants.newBuilder();
        pbPlants.setBearsFruit(bearsFruit);

        PBFlora.Builder pbFlora = pbPlants.getFloraBuilder();
        pbFlora.setIscultivated(isCultivated);

        PBProduct.Builder pbProduct = pbFlora.getProductBuilder();
        ...
        message.setPlants(pbPlants);

    };
    @Override public void copyFromProto(PBStockItem message){
        PBPlants pbPlants = message.getPlants();
        name = pbPlants.getFlora().getProduct().getName();
        cost = pbPlants.getFlora().getProduct().getCost();
        supplier = pbPlants.getFlora().getProduct().getSupplier();
        packaging.setUnits(pbPlants.getFlora().getProduct().getPackaging().getUnits());
        packaging.setCostPerUnit(pbPlants.getFlora().getProduct().getPackaging().getCostPerUnit());
        isCultivated = pbPlants.getFlora().getIscultivated();
        bearsFruit = pbPlants.getBearsFruit();
    };
}
