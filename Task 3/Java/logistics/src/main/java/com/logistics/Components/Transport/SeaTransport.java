...
import com.logistics.Serialization.PBLogistics.PBSeaTransport;
...
public class SeaTransport implements Transport {
    ...
    @Override public void copyToProto(PBTransportItem.Builder message){
        PBSeaTransport.Builder pbSeaTransport = PBSeaTransport.newBuilder();
        pbSeaTransport.setShipname(shipName);


        PBTransport.Builder pbTransport = pbSeaTransport.getTransportBuilder();
        pbTransport.setCode(code);
        pbTransport.setSpeed(speed);
        pbTransport.setCostPerMeter(costPerMeter);
        pbTransport.setRoute(route);

        message.setSeaTransport(pbSeaTransport);

    };
    @Override public void copyFromProto(PBTransportItem message){
        PBSeaTransport pbSeaTransport = message.getSeaTransport();
        code = pbSeaTransport.getTransport().getCode();
        speed = pbSeaTransport.getTransport().getSpeed();
        costPerMeter = pbSeaTransport.getTransport().getCostPerMeter();
        route = pbSeaTransport.getTransport().getRoute();
        shipName = pbSeaTransport.getShipname();
    };
}
