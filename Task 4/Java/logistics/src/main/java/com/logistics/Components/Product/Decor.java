...
import com.logistics.Serialization.PBLogistics.PBDecor;
...
public class Decor implements Furniture {
    ...
    @Override public void copyToProto(PBStockItem.Builder message){
        PBDecor.Builder pbDecor = PBDecor.newBuilder();
        pbDecor.setStyle(style);

        PBFurniture.Builder pbFurniture = pbDecor.getFurnitureBuilder();
        ...
        message.setDecor(pbDecor);

    };
    @Override public void copyFromProto(PBStockItem message){
        PBDecor pbDecor = message.getDecor();
        name = pbDecor.getFurniture().getProduct().getName();
        cost = pbDecor.getFurniture().getProduct().getCost();
        supplier = pbDecor.getFurniture().getProduct().getSupplier();
        packaging.setUnits(pbDecor.getFurniture().getProduct().getPackaging().getUnits());
        packaging.setCostPerUnit(pbDecor.getFurniture().getProduct().getPackaging().getCostPerUnit());
        material = pbDecor.getFurniture().getMaterial();
        style = pbDecor.getStyle();
    };
}
